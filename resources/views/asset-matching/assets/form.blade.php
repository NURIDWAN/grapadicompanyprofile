        ...
        <form id="asset-form" method="POST" enctype="multipart/form-data" action="{{ $editing ? route('matching.assets.update', $asset) : route('matching.assets.store') }}" class="space-y-6">
            ...
            <section id="asset-photo-uploader" data-existing-count="{{ $editing ? $asset->photos->count() : 0 }}" class="rounded-2xl border border-border-dark bg-surface-dark p-5 sm:p-7">
                ...
                @if($editing && $asset->photos->isNotEmpty())
                    @php($photosMarkedForDeletion = array_map('intval', (array) old('delete_photo_ids', [])))
                    <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        @foreach($asset->photos as $photo)
                            <div data-existing-photo class="overflow-hidden rounded-xl border border-border-dark bg-background-dark transition">
                                <div class="relative bg-black">
                                    <img src="{{ asset('storage/'.$photo->path) }}" alt="{{ $photo->alt_text ?: $asset->name }}" class="h-40 w-full object-cover transition">
                                    <span data-delete-status class="absolute left-2 top-2 hidden rounded-full bg-red-500/90 px-3 py-1 text-xs font-semibold text-white">Akan dihapus</span>
                                </div>
                                <div class="space-y-3 p-3">
                                    <label class="text-xs text-gray-400">Alt Text<input name="existing_photo_alt[{{ $photo->id }}]" maxlength="180" value="{{ old('existing_photo_alt.'.$photo->id, $photo->alt_text ?: $asset->name) }}" class="form-input !mt-1 !py-2 text-xs"></label>
                                    <input data-delete-existing type="checkbox" name="delete_photo_ids[]" value="{{ $photo->id }}" @checked(in_array($photo->id, $photosMarkedForDeletion, true)) class="hidden">
                                    <button data-delete-existing-button type="button" aria-pressed="false" class="flex w-full items-center justify-center gap-2 rounded-lg border border-red-500/40 bg-red-500/10 px-3 py-2 text-xs font-semibold text-red-300 transition hover:bg-red-500/20">
                                        <span class="material-icons-outlined text-base">delete</span>
                                        <span data-delete-label>Hapus</span>
                                    </button>
                                </div>
                            </div>
                        @endforeach
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateExistingCards=()=>{checks.forEach(check=>{const card=check.closest('[data-existing-photo]'), button=card.querySelector('[data-delete-existing-button]'), marked=check.checked;card.classList.toggle('!border-red-500/50',marked);card.querySelector('img').classList.toggle('opacity-40',marked);card.querySelector('[data-delete-status]').classList.toggle('hidden',!marked);card.querySelector('[data-delete-label]').textContent=marked?'Batalkan':'Hapus';button.querySelector('.material-icons-outlined').textContent=marked?'undo':'delete';button.setAttribute('aria-pressed',String(marked))})};
